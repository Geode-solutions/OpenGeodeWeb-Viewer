...
class VtkModelSurfacesPointsView(VtkModelView):
    ...
    @exportRpc(
        ...
        + model_surfaces_points_schemas_dict["visibility"]["rpc"]
    )
    def setModelSurfacesPointsVisibility(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_surfaces_points_schemas_dict["visibility"],
            self.model_surfaces_points_prefix,
        )
        params = schemas.Visibility.from_dict(rpc_params)
        self.SetPointsVisibility(params.id, params.visibility)

    @exportRpc(
        model_surfaces_points_prefix + model_surfaces_points_schemas_dict["size"]["rpc"]
    )
    def setModelSurfacesPointsSize(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_surfaces_points_schemas_dict["size"],
            self.model_surfaces_points_prefix,
        )
    ...
    @exportRpc(
        model_surfaces_points_prefix
        + model_surfaces_points_schemas_dict["color"]["rpc"]
    )
    def setModelSurfacesPointsColor(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_surfaces_points_schemas_dict["color"],
            self.model_surfaces_points_prefix,
        )
