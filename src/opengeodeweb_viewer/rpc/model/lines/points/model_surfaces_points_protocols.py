...
class VtkModelLinesPointsView(VtkModelView):
    ...
    @exportRpc(
        model_lines_points_prefix + model_lines_points_schemas_dict["visibility"]["rpc"]
    )
    def setModelLinesPointsVisibility(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_lines_points_schemas_dict["visibility"],
            self.model_lines_points_prefix,
        )
        params = schemas.Visibility.from_dict(rpc_params)
        self.SetPointsVisibility(params.id, params.visibility)

    @exportRpc(
        model_lines_points_prefix + model_lines_points_schemas_dict["size"]["rpc"]
    )
    def setModelLinesPointsSize(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_lines_points_schemas_dict["size"],
            self.model_lines_points_prefix,
        )
    ...
    @exportRpc(
        model_lines_points_prefix + model_lines_points_schemas_dict["color"]["rpc"]
    )
    def setModelLinesPointsColor(self, rpc_params: RpcParams) -> None:
        validate_schema(
            rpc_params,
            self.model_lines_points_schemas_dict["color"],
            self.model_lines_points_prefix,
        )
